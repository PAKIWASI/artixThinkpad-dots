local is_on = false
local keys_attached = false

-- lazy loading dap stuff on a keymap


vim.keymap.set({ "n", "x" }, "<leader>dd", function()

    if is_on then
        -- session_active check lives here so we never strand a running
        ...
        if keys_attached then
            for _, lhs in ipairs({
                "<leader>db", "<leader>dB", "<leader>dc", "<leader>dC",
                "<leader>dg", "<leader>di", "<leader>dj", "<leader>dk",
                "<leader>dl", "<leader>do", "<leader>dO", "<leader>dp",
                "<leader>dr", "<leader>ds", "<leader>dt", "<leader>dw",
                "<leader>de", "<leader>du",
            }) do
        ...
        -- require() only needed once -- it caches modules, so a second
        -- "on" press just skips straight past this if block
        if not keys_attached then
